...
class GANModel(BaseConditionalGenerationOracle):
    def __init__(self,
        ...
        self._cond_mean = torch.zeros(self._x_dim + self._psi_dim).float().to(device)
        self._cond_std = torch.ones(self._x_dim + self._psi_dim).float().to(device)
        self._y_mean = torch.zeros(self._y_dim).float().to(device)
        self._y_std = torch.ones(self._y_dim).float().to(device)
